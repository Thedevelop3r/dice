...
/// The Luck Bet table: eight labelled dice, winners lit green.
pub fn table_block(theme: &Theme, values: &[u32], labels: &[char], hits: &[bool], gold: bool) -> Vec<String> {
...
/// Same idea for the 8-die Luck Bet table, where every die always rolls.
/// `delays` controls the frame count/pacing — pass `&ROLL_FRAME_DELAYS`
/// (via `standard_frames()`) for the normal one-second settle, or
/// `&TURBO_FRAME_DELAYS` for the sustained Turbo spin.
pub fn animate_table_roll<F: FnMut(&mut Screen, &[u32; 8])>(
    screen: &mut Screen,
    rng: &mut Rng,
    delays: &[u64],
    mut draw_frame: F,
) -> [u32; 8] {
    let mut final_values = [0u32; 8];
